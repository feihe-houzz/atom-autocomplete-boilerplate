...
class ConnectProvider {
	...
	findMatchingSuggestions(prefix) {
		return new Promise((resolve) => {
            const resolverMapLocation = atom.config.get('graphql-annotation-autocomplete.resolverMapLocation');
            const resolverMap = require(resolverMapLocation);
            const resolverQueryMap = resolverMap.Query;
			...
			let matchingSuggestions = Object.entries(resolverQueryMap).filter((val) => {
